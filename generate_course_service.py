#!/usr/bin/env python3
"""Generate CourseService.cs with proper escaping for C# verbatim strings."""

import os

OUTPUT_PATH = os.path.join(
    os.path.dirname(__file__),
    "backend/MLOpsRoadmap.Api/CourseService.cs"
)


def esc(text: str) -> str:
    """Escape text for use in C# verbatim string (@"...")."""
    return text.replace('"', '""')
...
PLATFORM = """\
# Platform & Infrastructure for ML
...
CONTENTS = {
    ...
    "python-fundamentals": ("PythonContent",          PYTHON_FUND),
    "ml-basics":           ("MlBasicsContent",        ML_BASICS),
    "mlops-tools":         ("MlopsToolsContent",      MLOPS_TOOLS),
    "data-engineering":    ("DataEngContent",          DATA_ENG),
    "model-training":      ("ModelTrainingContent",   MODEL_TRAINING),
    "model-serving":       ("ModelServingContent",    MODEL_SERVING),
    "cicd":                ("CicdContent",            CICD),
    "monitoring":          ("MonitoringContent",      MONITORING),
...
CONST_FIELDS = "\n".join(
    f'    private const string {v[0]} = @"{esc(v[1])}";\n'
...
print(f"Written CourseService.cs ({len(CS):,} bytes)")
